...
public class EditRequest extends AbstractEditController {
    ...
    private static final Logger _logger = Logger.getLogger("EditRequest");
    @Inject
    ...
    private boolean requestIsComplete() {
        ...
        newTopic = checkField(newTopic, request.getAlternativeSolution(), "lblAlternativeSolution", "form:alternativeSolution", RequestTabs.tabReqSolutions);
        ...
        newTopic = checkField(newTopic, request.getRelevanceCurrent(), 1, null, "lblCurrentYear", "form:relevanceCurrent", RequestTabs.tabReqRelevance);
        newTopic = checkField(newTopic, request.getRelevancePast(), 1, null, "lblPastYear", "form:relevancePast", RequestTabs.tabReqRelevance);
        newTopic = checkField(newTopic, request.getRelevanceHospitals(), 1, null, "lblRelevanceHospitalCount", "form:relevanceHospitals", RequestTabs.tabReqRelevance);
        newTopic = checkField(newTopic, request.getRelevanceReason(), "lblRelevanceReason", "form:relevanceReason", RequestTabs.tabReqRelevance);

        if (request.getDocuments().size() > 0 || request.getDocumentsOffline().length() > 0) {
            newTopic = checkField(newTopic, request.isAnonymousData() ? "true" : "", "lblAnonymousData", "form:anonymousData", RequestTabs.tabReqDocuments);
        }
        if (!_msg.isEmpty()) {
