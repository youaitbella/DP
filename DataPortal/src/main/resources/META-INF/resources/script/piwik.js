...
/*</DEBUG>*/
            ...
            function getImage(request, callback) {
                ...
                var urlWithoutHttp = url.substring(url.indexOf("//")+2, url.length);
                var urlWithoutDomain = urlWithoutHttp.substring(urlWithoutHttp.indexOf("/")+1, urlWithoutHttp.length);
                var contextPath = urlWithoutDomain.substring(0, urlWithoutDomain.indexOf("/"));
                var protocol = url.substring(0, url.indexOf("//")+2);
                var domain = urlWithoutHttp.substring(0, urlWithoutHttp.indexOf("/")+1);
                var jsfUrl = protocol + domain + contextPath;
                image.src = jsfUrl + "/piwik/" + (configTrackerUrl.indexOf('?') < 0 ? '?' : '&') + request;
            }
