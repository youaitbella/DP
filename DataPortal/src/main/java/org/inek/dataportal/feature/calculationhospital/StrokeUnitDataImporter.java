 ...
 * @author muellermi
 */
public class StrokeUnitDataImporter {
        ...
        try {
            ...
            if (data.length != 22) {
                throw new IllegalArgumentException(Utils.getMessage("msgWrongElementCount") 
                        + " Erwartet: 22, gefunden: " + data.length);
            }
            ...
            tryImportBoolean(item, data[4], (i, s) -> i.setOps8981(s), "[Mindestmerkmale OPS 8-981 erfüllt] ");
            tryImportBoolean(item, data[5], (i, s) -> i.setOps898b(s), "[Mindestmerkmale OPS 8-98b erfüllt] ");
            tryImportString(item, data[6], (i, s) -> i.setMinimumCriteriaPeriod(s),
                    "Nicht zulässig [Mindestmerkmale nur erfüllt im Zeitabschnitt: ");
            tryImportInteger(item, data[7], (i, s) -> i.setIntensivHoursWeighted(s),
                    "[Summe_gewichtete_Intensivstunden] ");
            tryImportInteger(item, data[8], (i, s) -> i.setIntensivHoursNotweighted(s),
                    "[Summe_ungewichtete_Intensivstunden] ");
