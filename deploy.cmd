...
if %PORTAL_TYPE%==admin (
	%JAVA% -jar %GF_CLI% --user admin --host vdataportal0%NUMBER% redeploy --force --keepstate=true --name=DataPortalAdmin DataPortalAdmin\target\DataPortalAdmin.war
	goto end
)	
if %PORTAL_TYPE%==calc (
...
if %PORTAL_TYPE%==drg (
	%JAVA% -jar %GF_CLI% --user admin --host vdataportal0%NUMBER% redeploy --force --keepstate=true --name=DataPortalDrg DataPortalDrg\target\DataPortalDrg.war
	goto end
)
if %PORTAL_TYPE%==insurance (
	%JAVA% -jar %GF_CLI% --user admin --host vdataportal0%NUMBER% redeploy --force --keepstate=true  --name=DataPortalInsurance DataPortalInsurance\target\DataPortalInsurance.war
	goto end
)
if %PORTAL_TYPE%==psy (
	%JAVA% -jar %GF_CLI% --user admin --host vdataportal0%NUMBER% redeploy --force --keepstate=true  --name=DataPortalPsy DataPortalPsy\target\DataPortalPsy.war
	goto end
)
